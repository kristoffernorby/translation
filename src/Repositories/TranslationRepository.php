...
class TranslationRepository extends Repository
{
     ...
     *  @return boolean
     */
    public function has_exact($locale, $group, $item, $namespace='*')
    {
    ...
    {
        $this->model->where('id', $id)->update(['unstable' => '0','missing'=>'0','updated_by'=> Auth::user()->id]);
    }
    /**
     *  Flag the entry with the given id as not missing.
     *
     *  @param  integer $id
     *  @return boolean
     */
    public function flagAsNotMissing($id)
    {
        $this->model->where('id', $id)->update(['missing'=>'0','updated_by'=> Auth::user()->id]);
    }

    /**
     *  Flag the entry with the given id as not reviewed.
     *
     *  @param  integer $id
     *  @return boolean
     */
    public function flagAsNotReviewed($id)
    {
        $this->model->where('id', $id)->update(['unstable' => '1','updated_by'=> Auth::user()->id]);
    }
    ...
    public function validate(array $attributes)
    {
        ...
        $rules     = [
            ...
            'namespace' => 'required',
            'group'     => 'required',
            'item'      => "required|unique:ubsuite.{$table},item,NULL,id,locale,{$locale},namespace,{$namespace},group,{$group}",
